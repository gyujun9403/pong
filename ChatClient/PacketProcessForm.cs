...
namespace csharp_test_client
{
    public partial class mainForm
    {
        ...
        void SetPacketHandler()
        {
            ...
            PacketFuncDic.Add(PACKET_ID.ROOM_READY_RES, PacketProcess_RoomReadyRes);            
            PacketFuncDic.Add(PACKET_ID.ROOM_READY_NOTIFY, PacketProcess_RoomReadyNtf);
            PacketFuncDic.Add(PACKET_ID.GAME_START_NOTIFY, PacektProcess_GameStartNtf);
        }
        ...
        void PacketProcess_RoomReadyRes(byte[] bodyData)
        {
            var responsePkt = new RoomReadyResPacket();
            responsePkt.FromBytes((byte[])bodyData);
            readyBtn = responsePkt.isReady;
            syncReadyBtn();
            var msg = $"레디 요청 결과:  {(ERROR_CODE)responsePkt.Result}";
            DevLog.Write(msg, LOG_LEVEL.TRACE);
        }

        void PacketProcess_RoomReadyNtf(byte[] bodyData)
        {
            var notifyPkt = new RoomReadyNtfPacket();
            notifyPkt.FromBytes((byte[])bodyData);
            string msg;
            if (notifyPkt.isReady == true)
            {
                msg = $"방에서 릴레이 받음. {notifyPkt.UserUniqueId} - Waiting...";
            }
            else
            {
                msg = $"방에서 릴레이 받음. {notifyPkt.UserUniqueId} - Uncheck READY";
            }
            DevLog.Write(msg, LOG_LEVEL.TRACE);
        }

        void PacektProcess_GameStartNtf(byte[] bodyData)
        {
            var notifyPkt = new GameStartNtfPacket();
            notifyPkt.FromBytes(((byte[])bodyData));

            //string msg = $"key값 {notifyPkt.key} - Uncheck READY";
            GameForm gameForm = new GameForm(notifyPkt.key);
            gameForm.ShowDialog();
            //DevLog.Write(msg, LOG_LEVEL.TRACE);
        }
